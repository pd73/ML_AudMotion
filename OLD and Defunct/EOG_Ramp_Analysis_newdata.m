...
% Update 10/21/2014 Tighten the analysis window to be just 500ms before and
% after arm movement, by raising the threshold for arm movement to 10 at line 509.
...
try % calculate the trail start positions and velocity
    ...
    start_time = find(abs(velocities.vspeakerAZ(200:end))>10,1)+100;
    ...
    if start_time < 501
        ...
        startplot=start_time-250; % starts the plor 20 samples prior to the arm movement
    end
    ...
    eyesAZvelon = m.ramp_speed + 30; % saccades if faster then 50
    eyesAZaccon = 1500;
    
    PURvelon = 2; % Adam had 20 here - smooth pursuit goes down to slower here
    PURaccon = 50;
