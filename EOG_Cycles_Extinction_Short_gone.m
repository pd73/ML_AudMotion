...
% Update 1/22/2015 Adding Export of traces into _trace.mat file
%% Branch 1/28/2015 to handle extinction
    ...
    try % mark saccades in red
        ...
        if meas.numGshifts > 0
            for j = 1:meas.numGshifts
                ...
                if meas.pursuitMovements_end(j)-30 < 1
                    prevSmooth = meas.positions.eyesAZ(1:meas.pursuitMovements_end(j));
                else
                    prevSmooth = meas.positions.eyesAZ(meas.pursuitMovements_end(j)-30:meas.pursuitMovements_end(j));
                end
                if meas.pursuitMovements_start(j+1)+30 > length(meas.positions.eyesAZ)
                    nextSmooth = meas.positions.eyesAZ(meas.pursuitMovements_start(j+1):end);
                else
                    nextSmooth = meas.positions.eyesAZ(meas.pursuitMovements_start(j+1):(meas.pursuitMovements_start(j+1)+30));
                end
...
disp([ 'Export finished. Check Mat file ' scalefilename ])
...
positions.eyesAZ=(allData.data.(trialname).eyesAZ_R)*scaleF+offsetF;
...
try % calculate the trail start positions and velocity
    ...
    start_time = find(abs(velocities.vspeakerAZ(5000:end))>1,1)+5000;
    ...
    if start_time < 3001
        ...
        startplot=start_time-3000; % starts the plor 20 samples prior to the arm movement
    end
    ...
    [velocities, accelerations]= calcva(positions,2);
    
    eyesAZvelon = 30; % saccades if faster then 50
    eyesAZaccon = 700;
    ...
    pursuitMovements = find(diff(pursuitMovements_ind)>10); % reduced this number from 20, allows shorter smooth to be counted
...
[trial,	Stim,	~,	Velocity,	Start_pos,	End_pos,	Excursion,	extT] = lookupextinction(plotThis);

if extT == 0
    title({allData.filename(1:end-9),['Arm motion and EOG for full record of excel trial ' trialname,': ' Stim, ', no extinction.'],...
        ['Velocity = ', num2str(Velocity), 'deg/sec, sweeping ' num2str(Excursion) 'degrees from ' num2str(Start_pos) ' to '  num2str(End_pos)],'Purple = Speaker, Solid Green = R eye, Dashed Green = L eye, White = Gaze, Red = 0'})
    else
 title({allData.filename(1:end-9),['Arm motion and EOG for full record of excel trial ' trialname,': ' Stim, '. Extinction@ ', num2str(extT) ' ms.'],...
      ['Velocity = ', num2str(Velocity), 'deg/sec, sweeping ' num2str(Excursion) 'degrees from ' num2str(Start_pos) ' to '  num2str(End_pos)], 'Purple = Speaker, Solid Green = R eye, Dashed Green = L eye, White = Gaze, Red = 0'})
end

%plot([extT+meas.start_time-2000,extT+meas.start_time-2000], [-30 30] , 'w')
%title({allData.filename(1:end-9),'Arm motion and Eye signal for full record', })
...
%              PursuitMotion = [PursuitMotion, segment'];
            % desaccade
            % get smooth segments before and after the saccade
            if meas.pursuitMovements_end(i-shortSeg)-30 < 1
                prevSmooth = positions.eyesAZ(1:meas.pursuitMovements_end(i-shortSeg));
            else
                prevSmooth = positions.eyesAZ(meas.pursuitMovements_end(i-shortSeg)-30:meas.pursuitMovements_end(i-shortSeg));
            end
            if meas.pursuitMovements_start(i+1-shortSeg)+30 > length(positions.eyesAZ)
                nextSmooth = positions.eyesAZ(meas.pursuitMovements_start(i+1-shortSeg):end);                
            else
                nextSmooth = positions.eyesAZ(meas.pursuitMovements_start(i+1-shortSeg):(meas.pursuitMovements_start(i+1-shortSeg)+30));
            end
...
conditionsList = {
1,'Visual','Yes',20,0,-40,40,29500;
2,'Visual','Yes',20,-40,30,70,69880;
3,'Visual','Yes',20,30,-25,55,21200;
4,'Visual','Yes',20,-25,40,65,34300;
5,'Visual','Yes',20,40,-30,70,52280;
6,'Visual','Yes',20,-30,25,55,24000;
7,'Visual','Yes',20,25,-40,65,39100;
8,'Visual','Yes',20,-40,30,70,18400;
9,'Visual','Yes',20,30,-30,60,61080;
10,'Auditory','Yes',20,-30,25,55,29500;
11,'Auditory','Yes',20,25,-40,65,69880;
12,'Auditory','Yes',20,-40,30,70,21200;
13,'Auditory','Yes',20,30,-25,55,34300;
14,'Auditory','Yes',20,-25,30,55,52280;
15,'Auditory','Yes',20,30,-40,70,24000;
16,'Auditory','Yes',20,-40,30,70,39100;
17,'Auditory','Yes',20,30,-30,60,18400;
18,'Auditory','Yes',20,-30,30,60,61080;
19,'Imaginary','Yes',20,30,-40,70,3000;
20,'Imaginary','Yes',20,-40,30,70,3000;
21,'Imaginary','Yes',20,30,-40,70,3000;
22,'Imaginary','Yes',20,-40,25,65,3000;
23,'Imaginary','Yes',20,25,-30,55,3000;
24,'Imaginary','Yes',20,-30,30,60,3000
};
